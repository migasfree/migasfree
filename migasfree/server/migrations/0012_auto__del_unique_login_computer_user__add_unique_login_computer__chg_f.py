...
def removeoldlogin():
    ...
    lastlogin_ids="""(select id from server_login LASTLOGIN where id=
       (select id from server_login WHERE computer_id=LASTLOGIN.computer_id ORDER BY date DESC LIMIT 1)
       )
    """
    try:
        #delete login registers
        cursor.execute("DELETE FROM server_login_attributes where login_id not in %s;" % lastlogin_ids )
        cursor.execute("DELETE FROM server_login where id not in %s ;" % lastlogin_ids )

        #delete Query "LAST LOGIN"
        cursor.execute("DELETE FROM server_query where name='LAST LOGIN' ;")
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_unique(u'server_login', ['computer_id', 'user_id'])

        # Adding unique constraint on 'Login', fields ['computer']
        db.create_unique(u'server_login', ['computer_id'])


        # Changing field 'Schedule.name'
        db.alter_column(u'server_schedule', 'name', self.gf('django.db.models.fields.CharField')(unique=True, max_length=50))
    def backwards(self, orm):
        # Removing unique constraint on 'Login', fields ['computer']
        db.delete_unique(u'server_login', ['computer_id'])

        # Adding unique constraint on 'Login', fields ['computer', 'user']
        db.create_unique(u'server_login', ['computer_id', 'user_id'])


        # Changing field 'Schedule.name'
        db.alter_column(u'server_schedule', 'name', self.gf('django.db.models.fields.CharField')(unique=True, max_length=50, null=True))
    models = {
        u'auth.group': {
